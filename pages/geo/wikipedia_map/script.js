...
settingsForm.addEventListener("submit", (e) => {
  ...
  // Validate and set resultsCount (max 100)
  ...
  if (isNaN(inputResultsCount) || inputResultsCount <= 0) {
    ...
    if (inputResultsCount > 500) {
      alert("Results count capped at 100.");
    }
    settings.resultsCount = Math.min(inputResultsCount, 500); // Cap at 100
  }
...
function populateAdminList(feature) {
  ...
  if (cityName) {
    ...
    cityLink.href = `https://en.m.wikipedia.org/wiki/${encodeURIComponent(
      wikiCity
    )}`;
  ...
  if (stateName) {
    ...
    stateLink.href = `https://en.m.wikipedia.org/wiki/${encodeURIComponent(
      wikiState
    )}`;
  ...
  if (countryName) {
    ...
    countryLink.href = `https://en.m.wikipedia.org/wiki/${encodeURIComponent(
      wikiCountry
    )}`;
...
map.on("click", (e) => {
  const { lng, lat } = e.lngLat; // Get the longitude and latitude from the click event

  // Fly to the clicked location with a zoom level of 12
  map.flyTo({
    center: [lng, lat], // Center the map on the clicked location
    zoom: 11,           // Set the zoom level to 12
    speed: 1.3,         // Adjust the speed of the animation (default is 1.2)
    curve: 1.42         // Adjust the animation curve (default is 1.42 for smooth movement)
  });

  // Optional: Log the clicked coordinates for debugging
  console.log(`Map clicked at: Longitude: ${lng}, Latitude: ${lat}`);
});
